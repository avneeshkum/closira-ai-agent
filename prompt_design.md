...
### 2.3 Strict JSON Mode Enforcement

* **JSON Object Mode Activation:** The prompt forces the model to treat the structured schema as a hard constraint rather than a conversational suggestion.
* **Context Isolation:** By eliminating instructions regarding markdown fences and explicitly forbidding text generation outside the JSON structure, the system prevents raw block leakage into the chat interface.
* This enables the frontend/backend to reliably parse stage, escalation flag, and lead data.
...
* **Deterministic Response Formatting:** Relies on Groq's JSON mode configuration (`{"type": "json_object"}`) to maintain structural integrity. Without this enforcement at the payload level, raw LLM reasoning logs can leak into UI chat bubbles.
